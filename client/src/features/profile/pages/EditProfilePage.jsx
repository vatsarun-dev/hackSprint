  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 rounded-3xl border border-zinc-200 dark:border-zinc-800 bg-white/85 dark:bg-zinc-900/60 p-6 md:grid-cols-[1fr_auto] md:items-end">
        <div>
          <p className="text-sm uppercase tracking-[0.22em] text-cyan-200/80">Profile editor</p>
          <h1 className="mt-3 text-3xl font-semibold">Build your public profile</h1>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-600 dark:text-zinc-400">
            Save your name, role, location, bio, skills, and avatar. Your public profile updates immediately.
          </p>
        </div>
        {user?.avatar ? (
          <img src={user.avatar} alt={user.name || "Profile"} className="h-20 w-20 rounded-2xl object-cover" />
        ) : null}
      </div>
      <Card>
        <form onSubmit={handleSubmit(onSubmit)} className="grid gap-5 lg:grid-cols-2">
          <div className="space-y-4">
            ...
            <Input type="file" accept="image/*" {...register("avatar")} className="pt-3" />
          </div>
          <div className="space-y-4">
            <Textarea placeholder="Tell the community what you build" {...register("bio")} className="min-h-40" />
            <Input placeholder="Skills, separated by commas" {...register("skills")} />
            <div>
              <p className="mb-3 text-sm text-zinc-600 dark:text-zinc-400">Top skills</p>
